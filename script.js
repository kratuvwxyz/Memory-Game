...
const gameStructure = (e) => {
	...
	for (let t = 0; t < e; t++) {
		...
		for (let r = 0; r < e; r++) {
			...
			let e = $("<button>")
				.attr("id", `idu${o}`)
				.addClass(`col btn btn-light btn-${colors[o]} btn-lg py-3 m-1 gameButton border-${colors[o]} border-3`)
				.text(textShow ? o : " ")
				.attr("value", o)
				.attr("onclick", "userGenerate(this)")
				.attr("onmousedown", "mouseDown(this)")
				.attr("onmouseup", "mouseUp(this)")
				.attr("ontouchstart", "mouseDown(this)")
				.attr("ontouchend", "mouseUp(this)");
			t.append(e);
		}
		$("#userBox").append(t);
	}
